...
$(document).ready(function() {
    ...
    function postMessage(inputID, courseID) {
        ...
        $.ajax({
            type: 'POST',
            data: { "text": $(inputID).val(), "courseID": courseID, "time": $.now() },            url: site + port + '/api/tweets',
            success: function() {
                $(inputID).val('');
                getAllMessages();
            },
    ...
    $("#input1").keyup(function(e) { if (e.which == 13) { $("#enter1").click(); } });

    $("#enter2").click(function() { postMessage('#input2', "SE 3313"); });
    $("#input2").keyup(function(e) { if (e.which == 13) { $("#enter2").click(); } });
